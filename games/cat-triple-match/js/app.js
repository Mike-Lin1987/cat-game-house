(function () {
  ...
  function startTimer() {
    stopTimer();
    timerId = setInterval(() => {
      if (!state || state.status !== 'playing' || document.hidden || elements.modal.open) return;
      state.elapsed += 1;
      elements.timer.textContent = Core.formatElapsedTime(state.elapsed);
      if (state.elapsed % 5 === 0) Storage.saveSession(Core.serializeSession(state));
    }, 1000);
  }
  ...
  function completeLevel() {
    ...
    openResult(level.number === 100 ? '星夜貓屋全數完成！' : '配對完成！',
      `${'★'.repeat(stars)}　用時 ${Core.formatElapsedTime(state.elapsed)}`, false);
  }
  ...
  document.querySelector('#hint-button').addEventListener('click', () => {
    if (state.toolRemaining.hint <= 0 || state.status !== 'playing') return;
  ...
  document.querySelector('#shuffle-button').addEventListener('click', () => {
    if (state.toolRemaining.shuffle <= 0 || state.status !== 'playing') return;
    ...
    for (let offset = 1; offset < Math.min(200, symbols.length); offset += 1) {
      ...
      const candidate = Core.cloneState(state);
      ids.forEach((id, index) => { candidate.symbolByTileId[id] = symbols[(index + offset) % symbols.length]; });
      const result = Solver.solveLevel(level, { state: candidate, maxNodes: 150000, deadlineMs: 40 });
      if (!result.solved) continue;
      candidate.history.push(Core.createUndoSnapshot(state));
      candidate.toolRemaining.shuffle -= 1; candidate.toolUsed.shuffle += 1;
      state = candidate; render([{ type: 'shuffle' }]); announce('中央卡牌已重新洗牌'); return;
    }
