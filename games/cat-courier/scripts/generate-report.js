...
function buildMarkdown(analysis) {
  ...
  const lines = [
    ...
    ...analysis.levels.map((level) => (
      `| ${level.id} | ${level.chapter} | ${level.size} | ${level.stopCount} | ${level.oneWayEdgeCount} | ${level.optimalSteps} | ${level.fuelLimit} | ${level.optimalSolutionCount} | ${level.difficultyScore} | ${level.solverNodes} | ${level.valid ? '通過' : '失敗'} |`
    )),
    '',
