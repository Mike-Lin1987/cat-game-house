...
遊戲小屋缺少以旋轉管線、替每隻貓送奶為核心的固定關卡遊戲。玩家需要一款能直接以 `file://` 離線開啟、手機與鍵盤皆可操作、具有 100 個可證明唯一嚴格答案關卡的原創作品。

## Solution

新增《貓咪鮮奶管線》。玩家旋轉管線，讓唯一鮮奶槽連到所有貓咪碗；只要每隻貓都喝到牛奶就會通關。關卡產生與驗證仍要求保存答案讓全部有效格形成沒有洩漏、沒有迴圈的連通樹。遊戲提供五章 100 個固定關卡、提示、復原、重玩、星級、進度保存、審核頁、關卡產生器、獨立求解器及驗證報告。
...
4. 身為玩家，我在所有貓咪碗都從鮮奶槽收到牛奶時就會過關，不受其他未通奶管線、洩漏或迴圈影響。
...
- Core 是純規則 public seam；玩家完成條件與關卡嚴格驗證條件使用不同純函式，Renderer 不判斷完成、洩漏、連通或迴圈。
